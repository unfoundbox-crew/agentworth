//! `archie docs`: generates `docs/REFERENCE.md` (and its JSON twin, `docs/reference.json`,
...
/// One clap subcommand, at any depth (e.g. both `archie config` and `archie config
/// set`).
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CliCommandDoc {
    /// Full invocation, e.g. `"archie config set"`.
    pub path: String,
    pub about: String,
...
pub fn build_reference_doc() -> ReferenceDoc {
    ...
    let mut cli = Vec::new();
    // `archie` is the name the docs teach, so it is the one the reference echoes; the same
    // binary also answers to `agentworth` and, for now, `agwt`.
    collect_commands(&root, "archie", &global_ids, &mut cli);
...
/// every depth (so `archie config` and `archie config set` both get an entry).
...
pub fn render_markdown(doc: &ReferenceDoc) -> String {
    ...
    let _ = writeln!(
        out,
        "_Generated by `archie docs --write` from v{} on {}. Do not edit by hand -- \
         regenerate with `archie docs --write` (see `apps/cli/src/commands/docs.rs`)._",
    ...
    let _ = writeln!(
        out,
        "Registered by `archie serve` under `http://localhost:<port>` (default port \
    ...
    let _ = writeln!(
        out,
        "Register once with `claude mcp add agentworth --scope user -- archie mcp` \
...
pub fn run_docs_command(format: &str, write: bool) -> Result<()> {
    ...
    if write {
        ...
        if !docs_dir.is_dir() {
            anyhow::bail!(
                "docs/ directory not found in the current directory -- run `archie docs \
