...
import { parseChangelog, parseDocs, parsePosts, parseReference, webRoot } from './content.mjs';
...
writeFileSync(out, JSON.stringify({ releases, posts, reference, docs, downloads }, null, 2));
...
console.log(
  `content: ${releases.length} releases, ${posts.length} posts, ` +
    ...
    `docs ${docs.learn.length} learn/${docs.specs.length} specs/${docs.research.length} research, ` +
