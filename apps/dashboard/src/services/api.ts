...
/**
 * GET /api/usage -> daily/weekly/monthly token burn & cost rollups.
 *
 * The route ignores any `?period=` query string entirely -- it always returns all three
 * rollups (see `UsageResponse` in `apps/cli/src/server/routes.rs`), most-recent period
 * first in each array. There is no per-period fetch on the wire.
 */
export async function fetchUsageRollups(): Promise<UsageResponse> {
  try {
...
/**
 * GET /api/blame -> file change lineage matching session histories.
 *
 * The route returns a bare JSON array (`Vec<BlameMatch>`), not a `{file_path, total_edits,
 * edits}` wrapper -- see `BlameResponse` in `../types`.
 */
...
export async function fetchCoverageMatrix(): Promise<CoverageMatrixResponse> {
  ...
  return {
    total_adapters: GROUNDED_CAPABILITY_MATRIX.length,
    detected_adapters: GROUNDED_CAPABILITY_MATRIX.filter((a) => a.detected).length,
