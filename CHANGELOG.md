...
- The sample floor below which a rate is suppressed now lives in one place, `agentworth_storage::OUTCOME_RATE_DEFAULT_MIN_N`, read by `stats outcomes`, the `stats_outcomes` tool and the new ladder. Under it, a rate and a cost render blank — never a zero, never a dash pretending to be data (#124).
