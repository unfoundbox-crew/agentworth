...
- **Sessions have a `kind`: `conversation` or `fleet_snapshot`.** One column, one migration (every existing row is a conversation). The stub predicates (`total_events > 1 AND total_tokens > 0`, and the scanner's near-empty rule) apply to conversations only; a snapshot is outside that population, hidden from the default `session list`, `stats` and `/api/traces`, and shown whole by `session list --kind fleet_snapshot` or `--adapter <snapshot adapter>`. Adapters declare it through `AgentAdapter::trace_kind`.
