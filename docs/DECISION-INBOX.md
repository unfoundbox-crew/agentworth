...
| `OutcomeKind` PascalCase/snake_case encoding fix | **Done on `fix/outcome-kind-encoding-mismatch`, not yet merged into the integration branch.** | Real bug, confirmed against a peer session's live 10,188-session index. 301 passed, 0 failed on lenovo (298 + 3 new tests). See findings below |
...
- `verdict_breakdown.real_verified_rate` — re-triaged 2026-09-01, see findings above. Not the same root cause as the `OutcomeKind` encoding bug (this function never touches the stored string, and the math shows it can't mathematically exceed 100%). The `1.09`-type reading is most likely a percentage correctly read as if it were a 0–1 fraction. Real, separate, lower-priority bug found in the same function: `limit: None` silently caps at 50 sessions instead of meaning "unlimited" — spawned as its own task.
