...
| #63 | Brand pack |
| #70 | Compaction badge in the session list, og.png in Geist Mono, manifest colours |
| #71 | Four usefulness specs and the internal capability matrix, measured |
| #72 | Trace responses compressed and paginated; MCP `session_get` caps events |
| #73 | Dashboard inspector paints in 500 events, streams the rest |
| #74 | `scan` backfills `prompt_preview` and other derived fields; matrix derives from the registry |
| #75 | MCP `outcome_rate` — verified-outcome rate by model, adapter, or repo |
| #76 | `inspect` takes a prefix; `doctor` and `matrix` join the design system |
| #77 | The handoff: `session_handoff`, `carry_forward`, `agentworth handoff`; `loose-ends` ported to Rust |
| #78 | `stats` and `usage` now count the same sessions |
| #80 | Archaeology pane wired to real data; `models_usage_count`/`tools_usage_count` confirmed already fixed |
| #81 | Exit-code gating: "tests passed" now requires exit 0, not just a test command having run |

**Archaeology pane dead-wiring — fixed (#80).** `ArchaeologyPane.tsx` read `stats.archaeology` off `/api/stats`, which never set that key; `compute_archaeology_highlights` was only reachable via the separate `/api/archaeology` route, so the pane always rendered its empty state. Fixed on the frontend only, not the backend: `/api/archaeology` already existed, already ran the (unbounded, deep-trace-inspecting) `compute_archaeology_highlights`, and already had full HTTP-level test coverage (`test_api_get_archaeology_highlights` in `apps/cli/tests/api_server_tests.rs`, asserting every field non-empty on a fixture index) — the bug was purely that nothing in the dashboard ever called it. Wiring the field into `/api/stats` instead was rejected: that handler is polled/loaded per pane-mount already, and `compute_archaeology_highlights` walks full traces for up to 40 candidate sessions, so folding it in would run that work on every stats load instead of only when the Archaeology pane is open.
Fix: `ArchaeologyPane.tsx` now calls a new `fetchArchaeology()` (`apps/dashboard/src/services/api.ts`) hitting `/api/archaeology` directly. The frontend `ArchaeologyData` type (`apps/dashboard/src/types/index.ts`) and `ArchaeologyPanel.tsx` were rewritten to match the real Rust `ArchaeologyHighlights` shape (`most_expensive_unsolved`/`longest_recovery_loop`/`most_frequent_model_switches`/`token_carbon_dating`) — the old type was pure mock shape (`most_expensive_task.tokens: "18.3M tokens"` as a pre-formatted string, an invented `weird_discoveries` list with a `severity: 'hilarious' | 'bizarre' | 'costly'` enum) that never matched anything the backend could produce. The bottom "weird discoveries" grid is replaced with the real token-carbon-dating timeline + adapter token breakdown; each of the three highlight cards falls back to a plain "not found yet" placeholder when its backend field is `null` (a legitimate real result, not an error — confirmed live against a real index where `most_expensive_unsolved`/`longest_recovery_loop` came back `null` and `most_frequent_model_switches` didn't). Removed the now-dead `archaeology?: ArchaeologyData` field from `AggregateStats` and the matching dead mock data in `mockData.ts` (verified nothing else in the dashboard imported it).
Verified: `npm run build` (tsc strict + vite build) clean. Live end-to-end against the real ~57MB `~/.agentworth/agentworth.db` index (2,960 sessions): ran the real published `npx agentworth@latest` 0.1.13 binary as the API server (`serve --port 3000`) and this branch's dashboard source via `vite dev` (proxying `/api` to it) — confirmed `/api/archaeology` returns real, non-empty data (a real `most_frequent_model_switches` with 14 switches across 5,319 model invocations) and the Archaeology pane renders it, including the null-highlight placeholder path. This only proves the frontend fix — the 0.1.13 binary's own bundled dashboard still has the old dead wiring until this ships in a release.
Also checked bug #2 from the same brief (`models_usage_count`/`tools_usage_count` zero-seeding, flagged low-priority in this doc on 2026-09-01): **already fixed, no code change needed.** `get_aggregate_stats` in `crates/storage/src/lib.rs` computes both from each session's real `models_used`/`tools_used` columns (themselves populated for real in `crates/schema/src/trace.rs`'s `recalculate_stats`), already has a passing HTTP-level test (`test_api_get_stats_empty_and_populated`), and returns real non-zero counts against the live 0.1.13 binary on the real index (confirmed via `curl /api/stats`). The zero-seeding this doc's 2026-09-01 entry described predates whichever fix (likely the same stub-count-mismatch or session-cap work above) closed it; the doc entry was stale.
...
## `agentworth version` / `agentworth update` findings (2026-09-01)

Direct request, not part of the backlog above: `--version`/`-v` (clap's builtin) already worked, but there was no `version` subcommand and no `update` subcommand at all (`agentworth update` errored "unrecognized subcommand"). Commit `6b4ede7`, merged.

**Design question that mattered most: what should `update` actually do, given this ships via npm?** Checked `packages/agentworth/lib/resolver.js` (the npm launcher) first, since the brief suspected its existing GitHub-release download logic might already be a reusable update mechanism. It isn't, on its own: `downloadAndExtractBinary` downloads whatever version is already pinned in the local `package.json` (used as a first-run bootstrap when no binary exists yet), never "the latest available" — there's no "latest" resolution anywhere in the JS. Rewriting the Rust binary to replace an npm-managed install is exactly the fragile trap the brief warned against (no reliable way for the binary to find or safely overwrite the npm package from inside itself). So `update` does neither: it only checks GitHub Releases live and tells the user the exact command to run. Install-source detection reuses a signal that already existed in the codebase for something else: `resolver.js`'s `run()` already sets `AGENTWORTH_LAUNCHER_ACTIVE=1` on its child (an anti-recursion guard, unrelated to versioning) — `update`/`version` just read it too. Also threaded a new `AGENTWORTH_NPM_VERSION` through the same spawn call (extracted into a small pure `buildChildEnv()` helper, tested) purely so the binary can report the npm package's own version, which it otherwise has no way to know.

**Checked which install methods are real before writing a single word of advice text, rather than trusting README.md.** README.md advertises five: npm, a `curl -fsSL https://agentworth.dev/install.sh | sh` script, a Homebrew tap, `cargo install agentworth-cli`, and an agent-skill installer. Live-checked each: npm and GitHub Releases both really work (confirmed against the real registry and API — see below). The other three don't: `curl .../install.sh` actually serves the marketing site's `index.html` (an SPA catch-all route, `content-type: text/html`, not a shell script — piping it into `sh` fails immediately), `brew install unfoundbox-crew/tap/agentworth` 404s (no such tap repo exists), and `crates.io` has no `agentworth-cli` or `agentworth` crate. None of the three are wired into `.github/workflows/release.yml` either (it only ever publishes to GitHub Releases and npm). Same bug class as the already-found fabricated `agentworth gym` command in PR #11's SKILL.md — invented capability documented as real. Not fixed here (out of scope for this task, flagging per the brief instead of a task chip): README.md's install table needs the same correction SKILL.md already got.

**Also found, same live-check pass: this checkout's own version is stale.** Workspace `Cargo.toml` and `packages/agentworth/package.json` both still say `0.1.5`, but the real npm registry and GitHub Releases are already at `0.1.9` (`release.yml`'s `version-gate` job keeps tag/Cargo.toml/package.json locked together at release time, so the two agree with each other — just not with this not-yet-released checkout). This is expected, not a bug: the "final version bump" row above is explicitly deferred until the whole session's work lands. Turned it into a real regression test instead of just a note (`test_is_newer_version_real_world_case`, `is_newer_version("0.1.5", "v0.1.9")`) and used it as the live manual-verification case: the compiled binary run for real on lenovo (no `--offline`) correctly reported "0.1.9 available (you have 0.1.5)" against the real GitHub API.

**GitHub Releases chosen over the npm registry as the "latest version" source**, matching `resolver.js`'s own convention (it downloads release binaries from GitHub, never queries npm for "latest"). Same source works regardless of how the user actually got the binary (npm, raw GitHub download, `cargo install`, source build), which the npm registry alone wouldn't.

**Both commands default to running the live check**, 2s timeout, graceful non-fatal fallback to a "couldn't check" line on any failure (unreachable, timeout, rate-limited, malformed response) — never a hang, never a non-zero exit from a failed network call. `--offline` opts out entirely on both. Considered gating the check behind a flag by default instead (mirroring `blunder`'s existing `--submit` opt-in, and this repo's zero-telemetry positioning) but read the brief's "consider whether it should also report... whether a newer version is available" as asking for this to be a real, on-by-default property of `version`, not something buried behind a flag nobody discovers — and a GET to a public releases endpoint carries no user data, unlike the telemetry concern that positioning is actually about. Flagging the alternative here in case Saurabh prefers the stricter opt-in shape.

**Testability**: the entire live-check decision path is a pure function (`evaluate_update_status`, takes an already-resolved `Result` — no I/O), and both commands' full text output are pure formatters (`format_version_report`/`format_update_report`, return `String`, never `println!` directly) — so every branch (up to date / update available / check failed / skipped, npm-launcher active / not) is unit-tested without touching the network, per the brief's requirement. The real `reqwest` call (`fetch_latest_release`) is the one function deliberately left untested in the automated suite; verified manually instead (see above, and the npm-launcher branch was also manually verified live: `AGENTWORTH_LAUNCHER_ACTIVE=1 AGENTWORTH_NPM_VERSION=0.1.9 agentworth update` against the real API correctly led with just `npm install -g agentworth@latest`).

**Found, not fixed, flagging per the brief**: `packages/agentworth/test/launcher.test.js`'s `formats missing binary error message with helpful installation hints` test was already red before this task touched anything (confirmed via `npm test` first) — it asserts `formatMissingBinaryMessage()` contains `brew install agentworth` and the curl install script, neither of which the real function emits (that function only ever mentions `cargo build`, `cargo install --path apps/cli`, the GitHub releases URL, and `AGENTWORTH_BIN`). Given the live-check above, the test's assertions are the wrong ones to fix toward — brew/curl aren't real install paths — so the right fix is correcting the test, not the message. Left alone; own task if wanted.

Verified on lenovo (build dir `~/builds/agentworth-versionupdate-6b4ede7`, isolated `CARGO_TARGET_DIR`, `flock`-serialized): `cargo build --workspace` exit 0 (2 pre-existing unused-import warnings, unrelated — same two already known from earlier entries above). `cargo test --workspace --no-fail-fast` exit 0, every crate green, 0 failed anywhere. 33 new Rust tests (25 unit in `apps/cli/src/commands/version_info.rs`, compiled into both the `agentworth` and `agwt` binary targets per this crate's existing pattern; 10 new `assert_cmd` integration tests in `apps/cli/tests/version_update_integration_tests.rs`, all network-free via `--offline`). JS side: `npm test` in `packages/agentworth` — 16/17 pass (up from 13/14 baseline: +3 new tests for the `buildChildEnv` env-threading, 0 new failures; the 1 pre-existing failure above is untouched).

## `agentworth stats` vs `agentworth usage` session-count mismatch (2026-09-02)

Same root cause as the `get_aggregate_stats`/`list_sessions_filtered` stub-count mismatch above, one hop further: that fix only reached `get_aggregate_stats` (`agentworth stats`). The three usage views (`v_daily_usage`/`v_weekly_usage`/`v_monthly_usage`, backing `agentworth usage` and `/api/usage`), `get_model_usage` (`/api/usage/models`), and `get_pacing_window` (`agentworth pace`, `/api/pacing`, the MCP `pacing_window` tool) all kept their own, older filter — `started_at > '2020-01-01'` only, no stub exclusion. `agentworth usage`'s reported total against a real index (10,329 rows, live `agentworth` binary) was 10,118, against `agentworth stats`' 2,960 for the same rows -- see the before/after table below for a directly-reproduced version of the same gap, run as SQL against a copy of that index rather than the live binary (this Mac can't run the Linux build). 2,875 of the 3,019 `claude_code` rows dated 2026-09-01 turned out to be stubs stamped with that day's scan.

**Fix**: `NON_STUB_SQL_PREDICATE` (already the shared constant for `get_aggregate_stats`/`list_sessions_filtered`) is now applied everywhere a session gets counted: the three usage views (dropped and recreated on every `initialize_schema` run rather than `CREATE VIEW IF NOT EXISTS`, since SQLite has no `CREATE OR REPLACE VIEW` and `IF NOT EXISTS` would leave an already-existing database's view SQL frozen at whatever it was first created with), `get_model_usage`'s JOIN, and all three of `get_pacing_window`'s queries (the main aggregate, active-adapters, active-models). Token sums are unaffected — a stub fails the predicate partly *because* `total_tokens = 0`, so only `session_count` changes anywhere.

**Scanner alignment**: `Scanner::run_scan` used to only skip storing a parse that normalized to literally zero events (PR #68's parse-time backstop) — a session with one event and zero tokens still got stored, still failed every one of the query-side fixes above, and stuck around in the raw index forever. First attempt used the full `NON_STUB_SQL_PREDICATE` (events AND tokens) as the store-time gate too — reverted after it broke three real tests on lenovo (`autopsy`'s and both `agwt audit --safety` CLI tests' own fixtures: a real, multi-event session whose assistant reply carries no `usage` field, so `total_tokens` is 0 despite genuine content). Excluding a session from an aggregate count and deleting it from the index outright are different stakes: audit/autopsy/blind-spots need to find exactly this "real activity, no captured token telemetry" population, querying with `include_stubs: true` for precisely that reason. Final fix: a separate, narrower `NEAR_EMPTY_EVENTS_SQL_PREDICATE` (`total_events <= 1`, no token check at all) and its Rust mirror `agentworth_storage::is_near_empty_session(total_events)`, applied right after the zero-events backstop — skips storing unless `ScanOptions::include_stubs` is explicitly set. `Storage::stub_sessions` (renamed from `zero_activity_sessions`) uses the same narrower predicate for the prune pass. Kept PR #68's two existing safety rails unchanged: pruning only runs on a full, unscoped scan (`custom_paths.is_empty()`), and a stub row is only deleted if its `(adapter, source_path)` no longer appears among sources any current adapter would actually enumerate again — a source still genuinely detected is presumed a real, if thin, session and left alone, matching #68's own "6 genuine (if under-parsed) Gemini CLI logs, correctly retained" precedent. New `--include-stubs` flag on `agentworth scan` (and `ScanOptions::include_stubs` generally) is the explicit opt-out. The two stub definitions now deliberately diverge: `NON_STUB_SQL_PREDICATE` (events AND tokens) decides what counts in `agentworth stats`/`usage` aggregates; `NEAR_EMPTY_EVENTS_SQL_PREDICATE` (events alone) decides what stays in the index at all.

New field on `ScanOptions` meant touching every one of its ~46 struct-literal call sites across the workspace (adapters' own unit tests construct it just to call `.enumerate()`, unrelated to this fix) — added `..Default::default()` to each rather than hand-listing the new field everywhere, since `ScanOptions` already derives `Default`.

**Found while auditing every `include_stubs: Some(true)` query-side caller for the same mismatch** (`watch.rs`, `blind_spots.rs`, `autopsy.rs`, `threat_digest.rs`, `agwt audit --safety`): `watch.rs`'s Loop Sentinel re-scans on every poll specifically to catch a still-growing, currently-thin transcript, then queries with `include_stubs: Some(true)` to see it while it's thin — if the scan step silently stopped storing that same thin session, the query would never find it until it grew past the predicate, defeating the point of watching live. Fixed by setting `include_stubs: true` on `watch.rs`'s own `ScanOptions`. `blind_spots.rs`/`autopsy.rs`/`threat_digest.rs`/`audit.rs` don't call `run_scan` themselves (they query whatever `agentworth scan` already indexed); this audit is what surfaced the token-check regression above (autopsy's and audit's own existing tests scan a real 2-4 event, 0-token fixture and expect to find it) — the narrower `NEAR_EMPTY_EVENTS_SQL_PREDICATE` fixes it for real, not just for these tests, since any real audit-relevant session with uncaptured token usage would otherwise have silently vanished from the index the moment it was scanned.

**Tests**: a storage test (`test_stats_and_usage_surfaces_agree_on_non_stub_population`) seeding one real session plus both stub shapes (one-event/zero-token, zero-event/zero-token) and asserting `get_aggregate_stats`, `get_daily_usage`, `get_monthly_usage`, and `get_model_usage` all count exactly the one real session — the one-event stub also carries a `session_model_usage` row so the `get_model_usage` JOIN's own gap is actually exercised, not just incidentally passing. A core test (`test_prune_stub_sessions_removes_only_undetectable_rows`) extending the existing zero-events-only prune test with a stale one-event row, proving both stub shapes get pruned once their source is undetectable while a still-valid thin source survives. A new core test (`test_scanner_does_not_store_one_event_stub_by_default_but_keeps_tokenless_multi_event_sessions`) proving both directions of the narrower store-time gate: a one-event session is skipped by default and kept with `include_stubs: true`, and a real two-event, zero-token session (the exact shape that broke `autopsy`/`audit`) is stored regardless. Fixed three pre-existing tests whose fixtures never set `total_events` (defaulting to 0, which the new pacing-window predicate now correctly excludes): `test_pacing_window_calculation` (storage), `test_api_get_pacing_endpoint` (HTTP layer), and `test_burn_alarm_threshold_triggering` (CLI) — the last one only surfaced on the second lenovo run, after the token-check regression above was fixed and this one, unrelated fixture gap became the last thing standing between it and green.

Before/after on a copy of the real `~/.agentworth/agentworth.db` (10,329 rows; cannot run the Linux binary from this Mac, so both columns are the same SQL run directly against the copy, "after" with the predicate added — not a live binary run). Ran the actual view/query SQL unlimited (no `LIMIT`) for an apples-to-apples sum, since `get_daily/weekly/monthly_usage`'s default row cap and `get_model_usage`'s `limit` param would otherwise make the "before" number in this table not match what `agentworth usage` itself prints for this same index:

| Query | Before | After |
| --- | --- | --- |
| `agentworth stats` population (`total_events > 1 AND total_tokens > 0`) | 2,960 | 2,960 (unchanged — already correct) |
| `v_monthly_usage` session-count sum, all period/adapter rows | 10,327 | 2,960 |
| `claude_code` sessions on 2026-09-01 | 3,019 | 144 (2,875 stubs removed) |
| `get_model_usage("day")` session-count sum, all period/model rows | 3,052 | 3,042 |

The `get_model_usage` drop is much smaller than the other three: it joins against `session_model_usage`, and all but 10 of the real index's stub sessions never recorded a per-model usage row at all (no model was ever meaningfully invoked), so most were already excluded by the join's own shape before this fix — the predicate closes the last 10.

Branch `fix/one-session-definition`. Verified on lenovo, second run after the token-check fix above: `cargo build --workspace` exit 0, `cargo test --workspace --no-fail-fast` exit 0, every crate green, 0 failed anywhere (the first run had caught the token-check regression and one pre-existing unbuilt-dashboard test gap, both since fixed). Clippy skipped by director instruction — CI doesn't gate on it, and this run's own clippy failure was a pre-existing `too_many_arguments` lint in `agentworth-schema` unrelated to this change.
