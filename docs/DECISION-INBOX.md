...
- `verdict_breakdown.real_verified_rate` — re-triaged, see findings above. Not the same root cause as the `OutcomeKind` encoding bug. Real, separate, lower-priority bug found in the same function: `limit: None` silently caps at 50 sessions instead of meaning "unlimited" — spawned as its own task.
