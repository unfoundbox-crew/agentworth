...
- **New, found while fixing the item above**: `apps/cli/src/server/routes.rs`'s `get_stats_handler` calls `list_sessions_filtered` with `Some(10000)`, not `None` — a real, explicit cap, not the same bug, but the same shape of problem at a higher ceiling. On an index the size of the peer's real one (10,188 sessions), `/api/stats`'s `outcome_distribution` is already undercounting today. Not fixed, flagged only — different call site, looked deliberate rather than accidental.
