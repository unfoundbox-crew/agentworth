...
- **`Cargo.lock` staleness — done**, on `fix/punchlist-lockfile-and-verdict-rate` (commit `b27645d`), not yet folded into `integrate/handoff-batch-1`. The original symptom (`agentworth-*` locked at `0.1.3` vs workspace `0.1.5`) turned out to already be fixed by `bf5a5ff` — not present on this branch. Investigation found the real, current problem instead: `notify`/`tokio-stream` (from `feat/sse-live-tail`) and `toml` (from `feat/cli-persisted-config`) were added to Cargo.toml files but never made it into Cargo.lock through the sequential merges, so `cargo build --workspace --locked` failed outright ("cannot update the lock file ... because --locked was passed"). Fixed with a plain `cargo build --workspace` (no `--locked`, no `cargo update`) on lenovo — added only the missing subtrees; the only change to an existing entry was one benign cfg(windows)-only edge (quinn-udp's windows-sys pointer moved between two versions that already both existed in the graph). Verified on lenovo: `cargo build --workspace --locked` exits 0, `cargo test --workspace --locked` — 298 passed, 0 failed, matching the pre-fix baseline exactly.
