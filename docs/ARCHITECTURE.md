...
| `crates/governor` | The synchronous gate behind `archie hook --gate`: the thrash halt and the session spend cap, evaluated against `policy.toml` and the transcript tail, one socket round trip, fails open (`docs/specs/governor.md`). Never routes and never calls a model. |
