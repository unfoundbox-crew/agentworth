...
**A pull request.** For the subset that is mechanically completable — a file
that was going to be updated, a flag that was going to be added — open the PR
and label it honestly. The owner's suggested label was `missed-by-opus`, and
the joke is load-bearing: a tool that names which model dropped the ball is
more useful and more trustworthy than one that pretends it was nobody.
...
3. Only then consider opening PRs, and only for the mechanically obvious.

Do not start at step 3. A PR generated from a misread intent costs more trust
