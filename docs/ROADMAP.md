...
Status 2026-09-12, after v0.1.23. Sources: `docs/specs/README.md`, `docs/DECISION-INBOX.md`,
...
| :- | :--- | :--- | :--- | :--- |
| P0 | Coverage page shows NO for every adapter | Shipped page that lies; fixture verification not wired to it | 400k | dashboard |
| P0 | `real_verified_rate` can exceed 1.0 | Scoring bug on a number the landing page quotes | 300k | scoring |
| P0 | `archie home` test red on headless Linux | Red on clean main; hides the next real red | 200k | cli |
| P0 | Spec index stale: `suspect-commits` is built, four specs missing from the table | The roadmap doc misleads every new session | 100k | docs |
| P0 | Search model download is silent; landing says "100% offline" | Air-gapped user gets a bare failure; copy overstates | 250k | search + web |
| P0 | Capability matrix stale: Codex numbers pre-#116, Antigravity absent, junk rows | The measured doc is wrong by its own admission | 300k | docs + rescan |
| P1 | Land #133 + #134 (Claude Code plugin, DSH plugin) | Both green, 24 commits behind, conflict on 4 files | 600k | plugins |
| P1 | Land #167 (state.vscdb adapter for Cursor and VS Code forks) | New adapter, needs fixture + PARSER_VERSION check | 500k | adapter |
| P1 | Land #135 + #138 (landing page, blog) | Content, waits on your read only | 100k | your call |
| P1 | `/api/traces/:id` 64MB uncompressed payload | UI stalls on large sessions | 300k | api |
