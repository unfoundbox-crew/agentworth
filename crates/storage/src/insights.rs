...
pub fn compute_insights(conn: &Connection) -> Result<Insights> {
    let (tool_buckets, tool_order) = query_tool_buckets(conn)?;
    Ok(Insights {
        schema_version: INSIGHTS_SCHEMA_VERSION,
        generated_at: Utc::now().to_rfc3339(),
        window: InsightsWindow {
            sessions: query_session_window(conn)?,
        },
        population: query_population(conn)?,
        volume: query_volume(conn)?,
        by_adapter: query_by_adapter(conn)?,
        ladder: query_ladder(conn)?,
        verified: query_verified(conn)?,
        ...
        calls_per_turn: query_calls_per_turn(conn)?,
        calls_per_turn_by_adapter: query_calls_per_turn_by_adapter(conn)?,
        turn_buckets: query_turn_buckets(conn)?,
        file_modifications: query_file_modifications(conn)?,
        top_repos: query_top_repos(conn)?,
        top_sessions: query_top_sessions(conn)?,
        session_size_buckets: query_session_size_buckets(conn)?,
        models: query_models(conn)?,
        models_totals: query_models_totals(conn)?,
        series: query_series(conn)?,
...
fn query_tool_buckets(
    ...
    let sql = format!(
        "SELECT tools_used FROM sessions WHERE {USABLE_PREDICATE} AND tools_used LIKE '{{%' AND tools_used!='{{}}'"
    );
...
fn query_session_window(conn: &Connection) -> Result<InsightsSessionWindow> {
    let sql =
        format!("SELECT MIN(started_at), MAX(started_at) FROM sessions WHERE {USABLE_PREDICATE}");
    let row = conn
        .query_row(&sql, [], |r| {
...
fn query_population(conn: &Connection) -> Result<InsightsPopulation> {
    ...
    let tools = query_one_i64(
        conn,
        format!(
            "SELECT COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} AND tools_used LIKE '{{%' AND tools_used!='{{}}'"
        ),
    )?;
    let usable = query_one_i64(
        conn,
        format!("SELECT COUNT(*) FROM sessions WHERE {USABLE_PREDICATE}"),
    )?;
...
fn query_volume(conn: &Connection) -> Result<InsightsVolume> {
    let sql = format!(
        "SELECT COUNT(*), COALESCE(SUM(tool_calls_count),0), COALESCE(SUM(user_messages_count),0), \
         ...
         FROM sessions WHERE {USABLE_PREDICATE}"
    );
    let row = conn
        .query_row(&sql, [], |r| {
            Ok(InsightsVolume {
...
fn query_by_adapter(conn: &Connection) -> Result<Vec<InsightsAdapterRow>> {
    let sql = format!(
        "SELECT adapter, COUNT(*), COALESCE(SUM(tool_calls_count),0), \
         ...
         FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1 ORDER BY 2 DESC"
    );
    let mut stmt = conn.prepare(&sql)?;
    let rows = stmt
        .query_map([], |r| {
...
fn query_ladder(conn: &Connection) -> Result<Vec<InsightsLadderRow>> {
    let sql = format!(
        "SELECT CASE WHEN primary_outcome IS NULL OR primary_outcome='' \
         ...
         FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1"
    );
    let mut stmt = conn.prepare(&sql)?;
    let mut rows = stmt
        .query_map([], |r| {
...
fn query_verified(conn: &Connection) -> Result<InsightsVerified> {
    let ladder = query_ladder(conn)?;
...
fn query_calls_per_turn(conn: &Connection) -> Result<InsightsCallsPerTurn> {
    let strict = format!(
        "SELECT SUM(tool_calls_count)*1.0/SUM(user_messages_count) FROM sessions WHERE {USABLE_PREDICATE}"
    );
    let heavy = format!(
        "SELECT AVG(tool_calls_count*1.0/nullif(user_messages_count,0)) FROM sessions WHERE \
         user_messages_count>0 AND total_events>10 AND {USABLE_PREDICATE}"
    );
    let strict = conn
...
fn query_calls_per_turn_by_adapter(conn: &Connection) -> Result<Vec<InsightsCallsPerTurnRow>> {
    let sql = format!(
...
fn query_turn_buckets(conn: &Connection) -> Result<Vec<InsightsBucketRow>> {
    let sql = format!(
        "SELECT CASE WHEN user_messages_count=0 THEN '0' WHEN user_messages_count=1 THEN '1' \
         ...
         ELSE '>50' END AS k, COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1"
    );
...
fn query_file_modifications(conn: &Connection) -> Result<InsightsFileMods> {
    let total = query_one_i64(conn, "SELECT COUNT(*) FROM file_modifications")?;
    let sessions_with = query_one_i64(
        conn,
        "SELECT COUNT(DISTINCT session_id) FROM file_modifications",
    )?;
    let by_action = query_pairs(
        conn,
        "SELECT action, COUNT(*) FROM file_modifications GROUP BY 1 ORDER BY 2 DESC",
    )?;
    let worktrees = conn
        .query_row(
            "SELECT COUNT(*), COUNT(DISTINCT session_id) FROM file_modifications WHERE file_path LIKE '%worktrees%'",
            [],
            |r| {
...
fn query_top_repos(conn: &Connection) -> Result<Vec<InsightsRepoRow>> {
    let sql = "SELECT substr(file_path, instr(file_path, '/code/')+6, \
               instr(substr(file_path, instr(file_path, '/code/')+6), '/')-1) AS repo, \
               COUNT(*), COUNT(DISTINCT session_id) \
               FROM file_modifications WHERE file_path LIKE '%/code/%' \
...
fn query_top_sessions(conn: &Connection) -> Result<Vec<InsightsTopSessionRow>> {
    let sql = format!(
        "SELECT session_id, adapter, started_at, total_tokens, total_events, tool_calls_count, \
         ...
         FROM sessions WHERE {USABLE_PREDICATE} ORDER BY total_tokens DESC LIMIT 8"
    );
    let mut stmt = conn.prepare(&sql)?;
    let rows = stmt
        .query_map([], |r| {
...
fn query_session_size_buckets(conn: &Connection) -> Result<Vec<InsightsBucketRow>> {
    let sql = format!(
        "SELECT CASE WHEN total_tokens<10000000 THEN '<10M' WHEN total_tokens<50000000 THEN '10-50M' \
         ...
         FROM sessions WHERE {USABLE_PREDICATE} AND total_tokens>1000 GROUP BY 1"
    );
    let counts: Vec<(String, i64)> = query_pairs(conn, &sql)?;
    let shape: [(&str, &str); 4] = [
        ("<10M", "< 10M"),
...
fn query_models(conn: &Connection) -> Result<Vec<InsightsModelRow>> {
    let sql = "SELECT model, COUNT(*), \
               COALESCE(SUM(input_tokens+output_tokens+cache_read_tokens+cache_creation_tokens),0), \
               COALESCE(SUM(cache_read_tokens),0), COALESCE(SUM(cache_creation_tokens),0), \
               COALESCE(SUM(input_tokens),0), COALESCE(SUM(output_tokens),0) \
               FROM session_model_usage GROUP BY 1 ORDER BY 2 DESC";
    let mut stmt = conn.prepare(sql)?;
    let rows = stmt
        .query_map([], |r| {
            Ok(InsightsModelRow {
...
fn query_models_totals(conn: &Connection) -> Result<InsightsModelTotals> {
    let row = conn
        .query_row(
            "SELECT COALESCE(SUM(input_tokens+output_tokens+cache_read_tokens+cache_creation_tokens),0), \
             COALESCE(SUM(cache_read_tokens),0) FROM session_model_usage",
            [],
            |r| {
...
fn query_series(conn: &Connection) -> Result<InsightsSeries> {
    let monthly_by_adapter = query_month_adapter(
        conn,
        format!("SELECT strftime('%Y-%m', started_at), adapter, COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1, 2"),
    )?;
    let sessions_by_month = query_pairs_i64(
        conn,
        &format!("SELECT strftime('%Y-%m', started_at), COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1 ORDER BY 1"),
    )?;
    let daily = query_pairs_i64(
        conn,
        &format!("SELECT date(started_at), COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1 ORDER BY 1"),
    )?;
    let outcome_by_month = query_month_outcome(
        ...
        format!(
            "SELECT strftime('%Y-%m', started_at) AS m, CASE WHEN primary_outcome IS NULL OR \
                 ...
                 COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} GROUP BY 1, 2 ORDER BY 1"
        ),
    )?;
    ...
    let verified_by_month = query_pairs_i64(
        conn,
        &format!(
            "SELECT strftime('%Y-%m', started_at), COUNT(*) FROM sessions WHERE {USABLE_PREDICATE} \
...
mod tests {
    ...
    fn compute_over_synthetic_index_matches_golden_counts() {
        ...
        // Session A: usable, heavy. 3 human turns, 6 tool calls (Bash x3, Read x2, Write x1),
        // two file writes under /code/, one model invocation, commit-observed outcome.
        ...
        // Session B: usable, one-shot, no outcome evidence. 1 turn, 2 tool calls.
