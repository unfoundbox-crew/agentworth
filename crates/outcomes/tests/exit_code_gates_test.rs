//! Rung 3 means "the tests passed", not "a test command was typed".
//!
//! Without a real exit code the classifier used to hand out `TestOrBuildPassed` at 0.70 from
//! the command string alone, so `cargo test` that died on a compile error scored the same as
//! `cargo test` that went green. These lock the line in place.
