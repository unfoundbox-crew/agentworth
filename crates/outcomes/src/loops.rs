//! Loop detection: repeated identical tool calls, and file-edit thrashing.
//!
//! Lifted out of `agentworth watch` (which still owns the live polling and the terminal
//! rendering) so the scanner can run the same detector while it indexes, and persist what it
//! finds. The detection itself is unchanged -- same thresholds, same resolution heuristic,
//! same types -- it just no longer lives behind a CLI binary that a library crate cannot
//! depend on.
...
/// How many identical consecutive tool calls count as a loop. The value `agentworth watch` has
/// always passed.
pub const DEFAULT_MAX_TOOL_REPEATS: usize = 3;

/// How many touches to one file count as thrashing. The value `agentworth watch` has always
/// passed.
pub const DEFAULT_MAX_FILE_REVISIONS: usize = 4;
...
mod tests {
    use super::*;
    use agentworth_schema::{FileActionType, Provenance, ToolCall};
    use chrono::Utc;
