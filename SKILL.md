...
- `archie session wake [--workspace PATH] [--repo KEY] [--redact] [--json]`: What you were doing — the checkout it stands in, the newest session's task, proof, loose ends, and the next step, in 30 lines. The first command to run cold or after a compaction.
...
- `archie hook`: Reads one harness lifecycle event (`PreToolUse`, `PostToolUse`, `Stop`, and others) as stdin JSON and forwards it to `archie serve`'s loopback socket (`~/.agentworth/archie.sock`), or the spool (`~/.agentworth/spool/`) when serve isn't running. Always exits 0 — never slows or blocks the agent. `archie hook print claude` prints the settings.json snippet to paste in; it never writes the file itself.
- `archie agent status [--json]`: Live state per session — `registered`, `working`, `idle`, `ended` — from the hook events, not a poll.
- `archie session drift [SESSION_ID | --last] [--json]`: Re-checks the files a session read against what's on disk now, and names who changed each one — another session on this machine, or "not an agent on this machine."
- `archie session anchors [SESSION_ID | --last] [--json]`: The join keys a session's own tool results carried — `run_id`, file hashes, pane id — for tying its work to what SpacePilot measured or MotionVector rendered.
...
- `archie mcp`: Starts a read-only MCP server over stdio (`session_list`, `session_show`, `repo_blame`, `stats_usage`, `window_show`, `agent_list`, `stats_outcomes`, `session_handoff`, `session_carry_forward`, `session_wake`, `session_forgotten`, `session_asks`, `repo_suspect`, `agent_status`, `session_drift`, `session_burn`), so a coding agent can query this machine's session index mid-session — open a session with `session_wake`, end one with `session_handoff`, and if it has compacted, recover what its own summaries dropped with `session_forgotten`. `session_asks` finds where a question's answer already landed, so it never needs re-asking. Before pushing, `repo_suspect` names the commits worth a second look. `agent_status` and `session_drift` are the loop's live questions: what state is every agent in, and what moved under a session since it last checked. Register once with `claude mcp add agentworth --scope user -- archie mcp`.
- `archie hook --gate`: The same hook binary, in synchronous mode, for `PostToolBatch` and `UserPromptSubmit` (and `PreToolUse` on Codex, where the harness exposes a block). One socket round trip, a 50ms budget, and it fails open — no `archie serve`, and the hook exits 0 and writes one spool line rather than blocking the agent. `archie hook print claude --govern` prints both the async recorders and the sync gates.
- `archie session burn [id]`: Live tokens, dollars, turns, cache-read share, and burn rate per minute for a session, from the same transcript tail the spend cap reads.
- `archie policy show|check|lift <session>|replay`: Show the active `policy.toml`, check one session against it, lift a suspended session's block, or replay the two rules over the index to tune thresholds. Backed by `governor_events`, `session_suspensions`, and per-turn `turn_usage`.
